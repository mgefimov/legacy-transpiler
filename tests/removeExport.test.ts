...
describe('removeExport', () => {
  it('exports const to _moduleExports', async () => {
    expect(await transpile(`export const foo = 1;`, { src, resolveModule })).toBe(
      `const foo = 1;\n${prefix} = {\n  foo\n};\n`
    );
  });

  it('exports function to _moduleExports', async () => {
    expect(await transpile(`export function greet() { return "hi"; }`, { src, resolveModule })).toBe(
      `function greet() {\n  return "hi";\n}\n${prefix} = {\n  greet\n};\n`
    );
  });

  it('exports class to _moduleExports', async () => {
    expect(await transpile(`export class Foo {}`, { src, resolveModule })).toBe(
      `class Foo {}\n${prefix} = {\n  Foo\n};\n`
    );
  });
  ...
  it('exports default expression to _moduleExports', async () => {
    expect(await transpile(`export default 42;`, { src, resolveModule })).toBe(
      `var __default = 42;\n${prefix} = {\n  default: __default\n};\n`
    );
  });

  it('exports default named function to _moduleExports', async () => {
    expect(await transpile(`export default function named() {}`, { src, resolveModule })).toBe(
      `function named() {}\n${prefix} = {\n  default: named\n};\n`
    );
  });

  it('exports default anonymous function to _moduleExports', async () => {
    expect(await transpile(`export default function() {}`, { src, resolveModule })).toBe(
  ...
  it('removes export all', async () => {
    expect(await transpile(`export * from './module';`, { src, resolveModule })).toBe('');
  });

  it('no assignment when no exports', async () => {
    expect(await transpile(`const x = 1;`, { src, resolveModule })).toBe('const x = 1;\n');
  });
